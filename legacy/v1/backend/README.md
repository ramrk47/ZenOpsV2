...
This backend is configured for a local Postgres DB named `maulya` with no password:
...
- `DATABASE_URL=postgresql+psycopg2://postgres@localhost:5432/maulya`
- `JWT_SECRET=...`
...
resp = client.post('/api/auth/login', data={'username': 'admin@maulya.local', 'password': 'password'})
