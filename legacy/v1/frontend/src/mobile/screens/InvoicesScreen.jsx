...
export default function InvoicesScreen() {
  ...
  return (
    <MobileLayout title="Invoices" subtitle={`${unpaidCount} unpaid`} primaryAction={{ label: 'Search', to: '/m/search' }}>
      ...
      <Section title="Cashflow Signals" subtitle="Mobile view of payment pressure and movement.">
        <div className="m-stat-grid">
          <Card className="m-stat-card">
            <p>Visible</p>
            <strong>{rows.length}</strong>
            <small>Invoices in the current filter set.</small>
          </Card>
          <Card className="m-stat-card">
            <p>Unpaid</p>
            <strong>{unpaidCount}</strong>
            <small>Invoices still carrying a balance.</small>
          </Card>
          <Card className="m-stat-card">
            <p>Balance Due</p>
            <strong>{formatMoney(outstandingTotal, rows[0]?.currency || 'INR')}</strong>
            <small>Outstanding amount remaining on-screen.</small>
          </Card>
          <Card className="m-stat-card">
            <p>Paid</p>
            <strong>{formatMoney(paidTotal, rows[0]?.currency || 'INR')}</strong>
            <small>Payments already recorded in this view.</small>
          </Card>
        </div>
      </Section>
      ...
      <Section title="Invoice List" subtitle={`${rows.length} items · ${titleCase(statusFilter === 'ALL' ? 'all statuses' : statusFilter)}`}>
        ...
        {!loading && rows.length === 0 ? (
          <MobileEmptyState
            title="No invoices"
            body="No invoices match current filters."
            action={<button type="button" className="m-link-btn" onClick={() => setStatusFilter('ALL')}>Reset filters</button>}
          />
        ) : null}
          ...
          {rows.map((row) => (
            <button key={row.id} type="button" className="m-list-card" onClick={() => openInvoice(row)}>
              ...
              <div className="m-list-keyline">
                ...
                <span>Paid {formatMoney(row.amount_paid, row.currency)}</span>
                <span>Balance {formatMoney(row.amount_due, row.currency)}</span>
              </div>
            </button>
          ))}
        </div>
        ...
        {selectedInvoice ? (
          <div className="m-sheet-stack">
            <Card className="m-sheet-card">
              ...
              <KVRow label="Status" value={titleCase(selectedInvoice.status)} />
              <KVRow label="Assignment" value={selectedInvoice.assignment_code || selectedInvoice.assignment_id} />
              <KVRow label="Grand Total" value={formatMoney(selectedInvoice.grand_total, selectedInvoice.currency)} />
              <KVRow label="Paid" value={formatMoney(selectedInvoice.amount_paid, selectedInvoice.currency)} />
              <KVRow label="Balance" value={formatMoney(selectedInvoice.amount_due, selectedInvoice.currency)} />
              <KVRow label="Adjustments" value={formatMoney(selectedInvoice.adjustments_total, selectedInvoice.currency)} />
            </Card>
