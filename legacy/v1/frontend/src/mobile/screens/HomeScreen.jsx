...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const tasks = [
          fetchMobileSummary(),
          fetchAssignments({ completion: 'ALL', mine: true, limit: 20, sort_by: 'updated_at', sort_dir: 'desc' }),
        ]
  ...
  return (
    <MobileLayout
      title="My Day"
      subtitle={user?.full_name || user?.email || 'ZenOps'}
      primaryAction={{ label: 'Search', to: '/m/search' }}
    >
      {error ? <div className="m-alert m-alert-error">{error}</div> : null}

      <Section title="Priority Queue" subtitle="Role-aware snapshot for today">
        ...
        {!loading ? (
          <div className="m-card-grid">
            <QuickCard
              title="My Drafts"
              value={drafts.length}
              hint="Continue or submit pending draft work"
            ...
            <QuickCard
              title="Today / My Day"
              value={myDayCount}
              hint="Items that are overdue or due soon"
              to="/m/assignments?filter=active"
            />
            <QuickCard
              title="Uploads Missing"
              value={uploadsMissing}
              hint="Assignments missing checklist evidence"
              to="/m/uploads"
            />
            {canViewInvoices ? (
              <QuickCard
      ...
      <Section title="Queue" subtitle={`${queue.length} items`}>
        {!loading && queue.length === 0 ? (
          <MobileEmptyState title="No active queue" body="You are clear for now." />
        ) : null}
