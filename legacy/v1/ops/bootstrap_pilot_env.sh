...
APP_DOMAIN="${APP_DOMAIN:-app.maulya.in}"
...
log() { printf '[bootstrap-pilot-env] %s\n' "$*"; }
fail() { printf '[bootstrap-pilot-env][FAIL] %s\n' "$*" >&2; exit 1; }
write_file() {
  local path="$1"
  local content="$2"
...
log ""
log "Next:"
log "  1) Run: ./ops/deploy_pilot_v1.sh"
log "  2) Run: ./ops/smoke_v1_only.sh"
